...
const OBJECT_TYPE = '2-63307862'; 

// Display record
app.get('/', async (req, res) => {
    const url = `https://api.hubapi.com/crm/v3/objects/${OBJECT_TYPE}?properties=name,species,age`;
    try {
        const resp = await axios.get(url, {
            headers: { Authorization: `Bearer ${PRIVATE_APP_ACCESS}` }
        });
        const records = resp.data.results;
        res.render('home', { title: 'Custom Objects | Practicum', records });
    } catch (e) {
